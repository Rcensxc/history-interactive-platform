...
import {
  EMPTY_CITY_AI_EVENT_ID,
  generateEmptyCityAiStoryPackage,
  shouldUseEmptyCityAiMode,
  type EmptyCityAiStoryPackageRequest,
} from "@/lib/empty-city-ai";

export async function POST(request: Request) {
  const routeStart = performance.now();
  let payload: EmptyCityAiStoryPackageRequest | null = null;

  try {
    payload = (await request.json()) as EmptyCityAiStoryPackageRequest;
  } catch {
  ...
  if (
    !payload ||
    payload.eventId !== EMPTY_CITY_AI_EVENT_ID ||
    !shouldUseEmptyCityAiMode(payload.eventId, payload.viewpointId) ||
  ...
  const result = await generateEmptyCityAiStoryPackage(payload);
  ...
  console.info("[empty-city-ai][route]", {
