...
export function ScrapeBoneAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: ScrapeBoneAiStoryPlayerProps) {
  const baseViewpoint =
    ...
    null;
  const [scriptPackage, setScriptPackage] =
    useState<ScrapeBoneAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[scrape-bone-ai][client-render]", {
  ...
  const resolvedBackground = currentScene
    ...
    : enrichEventBackgroundAsset({
        eventId: eventItem.id,
        sceneId: "scrape-bone-healing-loading",
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[scrape-bone-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `scrape-bone-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/scrape-bone-healing/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as ScrapeBoneAiStoryPackageApiResponse;
      console.info("[scrape-bone-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[scrape-bone-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="scrape-bone-healing-package-loading"
        topActions={
          <>
            <Link
  ...
  return (
    ...
    >
      ...
      {currentScene.type === "decision" && currentScene.choices?.length ? (
        <div className="grid gap-3 px-4 pb-4 md:px-8">
          {currentScene.choices.map((choice) => (
            <button
              ...
              className="rounded-[22px] border border-white/12 bg-black/20 px-5 py-4 text-left text-sm leading-7 text-stone-100 transition hover:border-emerald-200/28 hover:bg-emerald-100/10"
            >
              {choice.label}
            </button>
          ))}
