...
export function BoilBeansAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: BoilBeansAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  const [scriptPackage, setScriptPackage] =
    useState<BoilBeansAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[boil-beans-ai][client-render]", {
  ...
  const resolvedBackground = currentScene
    ...
    : enrichEventBackgroundAsset({
        eventId: eventItem.id,
        sceneId: "boil-beans-package-loading",
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[boil-beans-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `boil-beans-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/boil-beans-burn-stalks/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as BoilBeansAiStoryPackageApiResponse;
      console.info("[boil-beans-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[boil-beans-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
