...
export function HeroesOverWineAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: HeroesOverWineAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  const [scriptPackage, setScriptPackage] =
    useState<HeroesOverWineAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[heroes-over-wine-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[heroes-over-wine-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `heroes-over-wine-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/heroes-over-wine/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as HeroesOverWineAiStoryPackageApiResponse;
      console.info("[heroes-over-wine-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[heroes-over-wine-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="heroes-over-wine-package-loading"
        topActions={
          <>
            <Link
