...
export function BearingThornsAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: BearingThornsAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  const [scriptPackage, setScriptPackage] =
    useState<BearingThornsAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[bearing-thorns-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[bearing-thorns-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `bearing-thorns-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/bearing-thorns-apology/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as BearingThornsAiStoryPackageApiResponse;
      console.info("[bearing-thorns-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[bearing-thorns-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="bearing-thorns-package-loading"
        topActions={
          <>
            <Link
