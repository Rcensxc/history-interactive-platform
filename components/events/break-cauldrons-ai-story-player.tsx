...
export function BreakCauldronsAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: BreakCauldronsAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  const [scriptPackage, setScriptPackage] =
    useState<BreakCauldronsAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[break-cauldrons-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[break-cauldrons-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `break-cauldrons-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/break-cauldrons-sink-boats/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as BreakCauldronsAiStoryPackageApiResponse;
      console.info("[break-cauldrons-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[break-cauldrons-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="break-cauldrons-package-loading"
        topActions={
          <>
            <Link
