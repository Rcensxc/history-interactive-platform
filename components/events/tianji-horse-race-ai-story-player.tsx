...
export function TianjiHorseRaceAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: TianjiHorseRaceAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  const [scriptPackage, setScriptPackage] =
    useState<TianjiHorseRaceAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[tianji-horse-race-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[tianji-horse-race-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `tianji-horse-race-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/tianji-horse-race/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as TianjiHorseRaceAiStoryPackageApiResponse;
      console.info("[tianji-horse-race-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[tianji-horse-race-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="tianji-horse-race-package-loading"
        topActions={
          <>
            <Link
        ...
        <div className="grid gap-3 px-4 pb-4 md:px-8">
          {currentScene.choices.map((choice) => (
            <button
              ...
              className="rounded-[22px] border border-white/12 bg-black/20 px-5 py-4 text-left text-sm leading-7 text-stone-100 transition hover:border-amber-200/28 hover:bg-amber-100/10"
            >
              {choice.label}
            </button>
          ))}
