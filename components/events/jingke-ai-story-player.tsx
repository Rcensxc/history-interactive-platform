...
export function JingkeAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: JingkeAiStoryPlayerProps) {
  const baseViewpoint =
    ...
    null;
  const [scriptPackage, setScriptPackage] = useState<JingkeAiScriptPackage | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[jingke-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[jingke-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `jingke-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/jingke-assassinates-qin/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as JingkeAiStoryPackageApiResponse;
      console.info("[jingke-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[jingke-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="jingke-package-loading"
        topActions={
          <>
            <Link
