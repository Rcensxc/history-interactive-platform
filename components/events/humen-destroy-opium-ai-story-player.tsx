...
type HumenDestroyOpiumAiStoryPlayerProps = {
  eventItem: HistoricalEvent;
  playableContent: EventPlayableContent;
  initialViewpointId: string;
};

type HumenDestroyOpiumAiStoryPackageApiResponse = {
  ok: boolean;
...
export function HumenDestroyOpiumAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: HumenDestroyOpiumAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[humen-destroy-opium-ai][client-render]", {
  ...
  const selectedViewpoint =
    ...
    content?.viewpoints.find((viewpoint) => viewpoint.isPlayable !== false) ??
  ...
  const resolvedBackground = currentScene
    ...
    : enrichEventBackgroundAsset({
        eventId: eventItem.id,
        sceneId: "humen-destroy-opium-loading",
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[humen-destroy-opium-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `humen-destroy-opium-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/humen-destroy-opium/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as HumenDestroyOpiumAiStoryPackageApiResponse;
      console.info("[humen-destroy-opium-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      if (payload.source === "fallback-local") {
        setStatusMessage(
          payload.warning ?? "AI 线性脚本生成失败，已切回本地静态剧情。",
        );
      } else if (payload.warning) {
        setStatusMessage(formatStoryWarningBanner(payload.warning));
      }
    } catch {
      console.warn("[humen-destroy-opium-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="humen-destroy-opium-package-loading"
        topActions={
          <>
            <Link
  ...
  return (
    <ImmersiveStageShell
      ...
      standee={showStandee ? <StageStandeeCard visual={activeVisual} /> : null}
      speakerBadge={
        showSpeakerName ? (
          <div className="inline-flex rounded-[16px] border border-emerald-200/18 bg-emerald-100/8 px-4 py-2">
            <p className="font-display text-lg text-emerald-50 md:text-xl">{currentScene.speaker}</p>
          </div>
        ) : undefined
      }
      ...
      footer={
        <>
          ...
          <StageProgressFooter
            ...
            continueLabel="继续 ›"
            showContinueHint={!isAwaitingChoice && !isFinished && !isLoading}
          />
        </>
      }
    >
      <div className="space-y-6">
        ...
        {currentScene.type === "decision" && currentScene.choices?.length ? (
          <div className="space-y-3">
            {currentScene.choices.map((choice) => {
              const activeChoice = currentChoice?.id === choice.id;
              return (
                <button
                  key={choice.id}
                  type="button"
                  ...
                  className={`w-full rounded-[24px] border px-5 py-4 text-left transition ${
                    activeChoice
                      ? "border-emerald-200/35 bg-emerald-100/12 text-stone-100"
                      : "border-white/10 bg-white/[0.03] text-stone-300 hover:border-white/20 hover:text-stone-100"
                  }`}
                >
                  <div className="text-xs uppercase tracking-[0.24em] text-stone-400/70">
                    现场选择
                  </div>
                  <div className="mt-2 text-base leading-7">{choice.label}</div>
                </button>
              );
            })}
          </div>
