...
export function ShenlongAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: ShenlongAiStoryPlayerProps) {
  const baseViewpoint =
    playableContent.viewpoints.find((viewpoint) => viewpoint.id === initialViewpointId) ??
    playableContent.viewpoints[0] ??
    null;
  const [scriptPackage, setScriptPackage] = useState<ShenlongAiScriptPackage | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[shenlong-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[shenlong-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `shenlong-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/shenlong-coup-eve/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as ShenlongAiStoryPackageApiResponse;
      console.info("[shenlong-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[shenlong-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="shenlong-package-loading"
        topActions={
          <>
            <Link
