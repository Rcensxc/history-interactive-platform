...
export function RedCliffsAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: RedCliffsAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[red-cliffs-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[red-cliffs-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `red-cliffs-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/battle-of-red-cliffs/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as RedCliffsAiStoryPackageApiResponse;
      console.info("[red-cliffs-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="red-cliffs-package-loading"
        topActions={
          <>
            <Link
