...
export function DebateWithWuAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: DebateWithWuAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[debate-with-wu-ai][client-render]", {
  ...
  const resolvedBackground = currentScene
    ...
    : enrichEventBackgroundAsset({
        eventId: eventItem.id,
        sceneId: "debate-with-wu-loading",
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[debate-with-wu-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `debate-with-wu-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/debate-with-wu-scholars/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as DebateWithWuAiStoryPackageApiResponse;
      console.info("[debate-with-wu-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[debate-with-wu-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="debate-with-wu-package-loading"
        topActions={
          <>
            <Link
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-[24px] border px-5 py-4 text-left transition ${
                    activeChoice
                      ? "border-amber-200/35 bg-amber-100/12 text-stone-100"
