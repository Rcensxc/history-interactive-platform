...
export function SmashWaterJarAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: SmashWaterJarAiStoryPlayerProps) {
  const baseViewpoint =
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[smash-water-jar-ai][client-render]", {
  ...
  const resolvedBackground = currentScene
    ...
    : enrichEventBackgroundAsset({
        eventId: eventItem.id,
        sceneId: "smash-water-jar-loading",
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[smash-water-jar-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `smash-water-jar-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/smash-water-jar/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as SmashWaterJarAiStoryPackageApiResponse;
      console.info("[smash-water-jar-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[smash-water-jar-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="smash-water-jar-package-loading"
        topActions={
          <>
            <Link
  ...
  return (
    <ImmersiveStageShell
      ...
      speakerBadge={
        showSpeakerName ? (
          <div className="inline-flex rounded-[16px] border border-[#b49d5f]/35 bg-[#b49d5f]/12 px-5 py-2">
            <p className="font-display text-lg text-stone-100 md:text-xl">
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-[24px] border px-5 py-4 text-left transition ${
                    activeChoice
                      ? "border-[#b49d5f]/45 bg-[#b49d5f]/16 text-stone-100"
