...
export function CupWineAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: CupWineAiStoryPlayerProps) {
  const baseViewpoint =
    ...
    null;
  const [scriptPackage, setScriptPackage] = useState<CupWineAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[cup-wine-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[cup-wine-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `cup-wine-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/cup-wine-release-power/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as CupWineAiStoryPackageApiResponse;
      console.info("[cup-wine-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[cup-wine-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="cup-wine-package-loading"
        topActions={
          <>
            <Link
