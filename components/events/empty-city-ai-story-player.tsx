...
export function EmptyCityAiStoryPlayer({
  eventItem,
  playableContent,
  initialViewpointId,
}: EmptyCityAiStoryPlayerProps) {
  const baseViewpoint =
    ...
    null;
  const [scriptPackage, setScriptPackage] = useState<EmptyCityAiScriptPackage | null>(null);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      console.info("[empty-city-ai][client-render]", {
  ...
  async function loadStoryPackage(triggerSource: ClientTriggerSource) {
    if (packageRequestInFlightRef.current) {
      console.info("[empty-city-ai][client-trigger]", {
        triggerSource,
        deduped: true,
      });
      return;
    ...
    const clientRequestId = `empty-city-package-${Date.now()}-${packageRequestCount}`;
    ...
    try {
      const response = await fetch("/api/events/empty-city-stratagem/scene", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as EmptyCityAiStoryPackageApiResponse;
      console.info("[empty-city-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
      ...
      console.warn("[empty-city-ai][client-response]", {
        requestId: clientRequestId,
        triggerSource,
        networkMs: Number((performance.now() - startedAt).toFixed(1)),
        source: "fetch-error",
  ...
  if (!content || !currentScene) {
    return (
      <ImmersiveStageShell
        ...
        sceneId="empty-city-package-loading"
        topActions={
          <>
            <Link
